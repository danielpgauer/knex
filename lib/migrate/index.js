...
var Migrator = (function() {
  ...
  Migrator.prototype._ensureTable = function _ensureTable() {
    ...
    return getSchemaBuilder(trx, schemaName)
      ...
      .then(function(exists) {
        return (
          !exists && _this5._createMigrationTable(tableName, schemaName, trx)
        );
      })
      .then(function() {
  ...
  Migrator.prototype._createMigrationTable = function _createMigrationTable(
    tableName,
    schemaName
  ) {
    var trx =
  ...
  Migrator.prototype._createMigrationLockTable = function _createMigrationLockTable(
    tableName
  ) {
    var trx =
      ...
      this._getLock(trx)
        ...
        .then(function() {
          return _bluebird2.default.all(
            (0, _lodash.map)(
              migrations,
              (0, _lodash.bind)(_this7._validateMigrationStructure, _this7)
            )
          );
        })
        .then(function() {
        ...
        .catch(function(error) {
          ...
          if (error instanceof LockError) {
            ...
            _this7.knex.client.logger.warn(
            ...
            _this7.knex.client.logger.warn(
              'If you are sure migrations are not running you can release the ' +
                ...
                'table: ' +
                _this7._getLockTableNameWithSchema()
            );
          } else {
            if (_this7._activeMigration.fileName) {
              _this7.knex.client.logger.warn(
                'migration file "' +
                  _this7._activeMigration.fileName +
                  '" failed'
              );
            }
            _this7.knex.client.logger.warn(
...
function warnPromise(value, name, fn) {
  if (!value || typeof value.then !== 'function') {
    this.knex.client.logger.warn(
      'migration ' + name + ' did not return a promise'
    );
